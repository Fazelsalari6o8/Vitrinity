...
const productsItem = document.querySelectorAll(".product-items-container-Tw");
const searchInput = document.querySelector(".search-input-Tw");

const productsExist = document.querySelector(".exist-frame-Tw");
...
const searchHandler = (event) => {
  ...
  const userSearch = event.target.value.trim();
  productsItem.forEach((product) => {
    // console.dir(product.dataset.exist);
    const productsName = product.children[1].children[0].innerText;
    const productsExisting = product.dataset.exist === "yes";
    if (productsName.includes(userSearch) && productsExisting) {
